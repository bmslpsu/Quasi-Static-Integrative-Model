...
[element, Lift_force, Drag_force] =LiftAndDragForces(element,psi_f,del_r,phi_dotf,1);
%%  Find the rotational force acting on each wing
% note: the input anglur velocity is in deg/s. it is converted to rad/s in
% the function.
[element1, Rot_force]=FindRotationalForce(element,psi_dotf,del_r,c,1);

%% Added mass force
[element2, Am_force]=FindAddedMass(element1,psi_dotf,psi_f,del_r,c,time,1);
...
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
function force_AM_lift=FindAMLift(element)

force_AM_lift=zeros(1,length(element(1).force_AM_vec(2,:)));
for j=1:length(element(1).force_AM_vec(2,:))
    Rot_wing_dummy=0;
    for i=1:length(element)
        Rot_wing_dummy=Rot_wing_dummy+element(i).force_AM_vec(2,j);
    end
    force_AM_lift(j)=Rot_wing_dummy;
end
figure
plot(force_AM_lift)
title('Lift added mass force')
end

function force_rot_lift=FindForceRotLift(element)

force_rot_lift=zeros(1,length(element(1).force_Rot_vec(2,:)));
for j=1:length(element(1).force_Rot_vec(2,:))
    Rot_wing_dummy=0;
    for i=1:length(element)
        Rot_wing_dummy=Rot_wing_dummy+element(i).force_Rot_vec(2,j);
    end
    force_rot_lift(j)=Rot_wing_dummy;
end
figure
plot(force_rot_lift)
...
function [element,AM_wing_force] =FindAddedMass(element,psi_dotf,psi_f,del_r,c,time,test)
global rho
for j=1:length(element)
    ...
    for i=1:length(linear_acc_filt1)
        ...
        part2=(dot(element(j).linear_vel(:,i),linear_acc_filt(:,i))*sign(psi_f(i))*sind(psi_f(i)))/element(j).linear_vel_norm(i);
        part3=(sign(psi_dotf(i))*element(j).linear_vel_norm(i)*psi_dotf(i)*cosd(psi_f(i)));
...
if test==1
    AM_wing_force=zeros(1,length(element(1).force_Lift));
    for j=1:length(element(1).force_AddedMass)
        AM_wing_dummy=0;
        for i=1:length(element)
            AM_wing_dummy=AM_wing_dummy+element(i).force_AddedMass(j);
        end
        AM_wing_force(j)=AM_wing_dummy;
    end
end
figure
plot(AM_wing_force)
...
function [element, Rot_wing_force] =FindRotationalForce(element,psi_dotf,del_r,c,test)
%del_r (m)
%c (m)
%C_r from lit (1.55)
...
if test==1
    Rot_wing_force=zeros(1,length(element(1).force_Lift));
    for j=1:length(element(1).force_Rotation)
        Rot_wing_dummy=0;
        for i=1:length(element)
            Rot_wing_dummy=Rot_wing_dummy+element(i).force_Rotation(j);
        end
        Rot_wing_force(j)=Rot_wing_dummy;
    end
end
figure
plot(Rot_wing_force)
...
function [element,Lift_wing_force,Drag_wing_force] =LiftAndDragForces(element,psi_f,del_r,phi_dotf,test)
...
for j=1:length(element)
    ...
    for i=1:length(element(j).linear_vel)
        %angle of attack is in degrees (sane 2002)
        C_L(i)=0.225+1.58*sind(2.13*abs(psi_f(i))-7.28);
        C_D=1.92-1.55*cosd(2.04*abs(psi_f(i))-9.82);
        element(j).force_Drag(i)=sign(phi_dotf(i))*0.5*c*rho*norm(element(j).linear_vel(:,i))^2*C_D*del_r*(-sign(psi_f(i))*sign(phi_dotf(i)));
        element(j).force_Lift(i)=0.5*c*rho*norm(element(j).linear_vel(:,i))^2*C_L(i)*del_r;
    end
...
if test==1
    ...
    Drag_wing_force=zeros(1,length(element(1).force_Lift));
    for j=1:length(element(1).force_Lift)
        Lift_wing_dummy=0;
        Drag_wing_dummy=0;
        for i=1:length(element)
            Lift_wing_dummy=Lift_wing_dummy+element(i).force_Lift(j);
            Drag_wing_dummy=Drag_wing_dummy+element(i).force_Drag(j);
        end
        Lift_wing_force(j)=Lift_wing_dummy;
        Drag_wing_force(j)=Drag_wing_dummy;
    end
end
figure 
...
hold on
plot(Drag_wing_force)
title('drag force for the whole wing throughout a stroke')

figure
Force_Translational=sqrt(Lift_wing_force.^2+Drag_wing_force.^2);
plot(Force_Translational)
title('Translation force as described by Sane and Dickinson 2002')
end
