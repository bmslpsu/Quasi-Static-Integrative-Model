...
element =LiftAndDragForces(element,psi_f,del_r,phi_dotf,1);
...
function element=LiftAndDragForces(element,psi_f,del_r,phi_dotf,test)
...
for j=1:length(element)
    ...
    for i=1:length(element(j).linear_vel)
        ...
        element(j).force_Drag(i)=sign(phi_dotf(i))*0.5*c*rho*norm(element(j).linear_vel(:,i))^2*C_D*del_r;
